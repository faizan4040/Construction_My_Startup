...
export async function POST(request) {
  try {
    ...
    if (documentFile && documentFile.size > 0) {
      ...
      const result = await uploadToCloudinary(buffer, "labour_documents")
    ...
    if (profileFile && profileFile.size > 0) {
      ...
      const result = await uploadToCloudinary(buffer, "labour_profiles")
      profileImageUrl = result.url
      profilePublicId = result.public_id
    }

    // ── Generate slug only if this is a NEW profile ───────────────
    // findOneAndUpdate bypasses pre("save"), so we must generate it here.
    // For existing profiles we keep their current slug untouched.
    const existing = await LabourProfile.findOne({ user: currentUser.id }).select("slug").lean()

    let slug = existing?.slug   // keep existing slug on updates

    if (!slug) {
      // New profile — generate a unique slug
      const base = slugify(`${firstName} ${lastName}`, { lower: true, strict: true })
      // Append a short unique suffix to avoid collisions between same-name users
