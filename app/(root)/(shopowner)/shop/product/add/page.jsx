...
const AddShopProduct = () => {
  ...
  // Categories are set up by Admin only — Shopowner just picks one.
  // Reusing the same shared, public category API used everywhere else,
  // so there's only ever ONE list of categories in the whole system.
  ...
  useEffect(() => {
    if (getCategory && getCategory.success) {
      ...
      // Products link to a SUBCATEGORY (leaf, e.g. "Cement") — that's where
      // the Weight/Diameter/Color attribute template Admin configured lives.
      // Top-level groups (e.g. "Civil & Interiors") are never selectable here.
          ...
          <CardContent className="p-4 sm:p-5 md:p-7">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 sm:space-y-8">
                ...
                {/* Category — Admin-defined subcategories only */}
                <section>
                        ...
                        <FormControl>
                          {categoryOption.length === 0 ? (
                            <div className="rounded-xl sm:rounded-2xl border-2 border-dashed border-gray-200 p-4 sm:p-6 text-center text-xs sm:text-sm text-gray-500">
                              No categories available yet. Ask Admin to set up a category for your products.
                            </div>
                          ) : (
