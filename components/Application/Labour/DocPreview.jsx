    ...
    return (
        <div className='bg-gray-50 rounded-2xl border overflow-hidden shadow-sm'>
            ...
            <div className='h-52 flex items-center justify-center bg-white'>
                ...
                {isImage && <img src={url} className='w-full h-full object-cover' alt={label} />}

                {isPdf && <iframe src={url} className='w-full h-full' />}
            </div>
            {url && (
                <a href={url} target="_blank" rel="noopener noreferrer" className="block text-center text-xs py-2 font-medium hover:bg-gray-100">
                    Open Full Document
                </a>
            )}
        </div>
