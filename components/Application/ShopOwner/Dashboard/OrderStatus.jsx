...
const OrderStatus = () => {
  ...
  if (loading) {
    return <div className="bg-white dark:bg-card rounded-2xl shadow-sm p-10 text-center text-sm text-gray-400">Loading order status...</div>
  }

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="bg-white dark:bg-card rounded-2xl shadow-sm p-5 flex flex-col">
        <ChartStyle id={id} config={chartConfig} />
        <div className="flex items-start justify-between mb-2">
          <div>
            <p className="text-xs text-gray-400 font-medium">Pipeline</p>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">Your Order Status</h2>
            <p className="text-xs text-gray-400 mt-0.5">Total: {totalCount}</p>
          </div>
        ...
        <div className="flex-1 flex justify-center items-center">
          {!chartData.length ? (
            <div className="py-10 text-sm text-gray-400">No orders yet</div>
          ) : (
